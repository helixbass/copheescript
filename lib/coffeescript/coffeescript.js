...
(function() {
  ...
  var FILE_EXTENSIONS, Lexer, SourceMap, babelTraverse, babylonToEspree, babylonTokenTypes, base64encode, checkShebangLine, compile, compileToBabylon, dump, espreeTokenTypes, extraTokensForESLint, flatten, formatSourcePosition, getEspreeTokenType, getSourceMap, helpers, isPlainObject, lexer, packageJson, parser, renderWithPrettier, sourceMaps, sources, tokensForESLint, traverseBabylonAst, withPrettyErrors,
  ...
  exports.compile = compile = withPrettyErrors(function(code, options = {}) {
    var encoded, filename, generateSourceMap, i, js, len, map, parsed, ref, ref1, sourceMapDataURI, sourceURL, token, tokens, transpiler, transpilerOptions, transpilerOutput, v3SourceMap;
    ...
    // dump {parsed}
    js = (() => {
      var currentColumn, currentLine, fragment, fragments, header, j, len1, newLines;
      if (options.usePrettier) {
        return renderWithPrettier(parsed, options);
      }
  ...
  renderWithPrettier = function(parsed, options) {
    return parsed.prettier(options);
  };

  exports.prettier = withPrettyErrors(function(source, options) {
    var parsed;
    parsed = typeof source === 'string' ? parser.parse(lexer.tokenize(source, options)) : parser.parse(source);
    return renderWithPrettier(parsed, options);
  });
  ...
  traverseBabylonAst = function(node, func) {
    var _, child, item, results;
    if (Array.isArray(node)) {
      return (function() {
        ...
        for (i = 0, len = node.length; i < len; i++) {
          item = node[i];
          results.push(traverseBabylonAst(item, func));
        }
        return results;
      })();
    }
    ...
    if (isPlainObject(node)) {
      ...
      for (_ in node) {
        ...
        child = node[_];
        results.push(traverseBabylonAst(child, func));
      }
      return results;
    }
  };
  ...
  exports.parseForESLint = function(code, opts) {
    var ast, tokens;
    opts.bare = true;
    ({ast, tokens} = compileToBabylon(code, _extends({}, opts, {
      withTokens: true
    })));
