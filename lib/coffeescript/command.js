...
(function() {
  ...
  var BANNER, CoffeeScript, EventEmitter, SWITCHES, baseFileName, buildCSOptionParser, compileJoin, compileOptions, compilePath, compileScript, compileStdio, ends, exec, extend, findDirectoryIndex, forkNode, fs, hidden, invertLiterate, isCoffee, isLiterate, joinTimeout, makePrelude, merge, mkdirp, notSources, optionParser, optparse, opts, outputPath, parseOptions, path, printLine, printTokens, printWarn, removeSource, removeSourceDir, silentUnlink, sourceCode, sources, spawn, timeLog, usage, useWinPathSep, util, version, wait, watch, watchDir, watchedDirs, writeJs,
    indexOf = [].indexOf;
  ...
  SWITCHES = [['--ast', 'generate an abstract syntax tree of nodes'], ['--ast-printer', 'generate JS via AST printer'], ['--babylon', 'print out Babylon AST'], ['-b', '--bare', 'compile without a top-level function wrapper'], ['-c', '--compile', 'compile to JavaScript and save as .js files'], ['-e', '--eval', 'pass a string from the command line as input'], ['-h', '--help', 'display this help message'], ['-i', '--interactive', 'run an interactive CoffeeScript REPL'], ['-j', '--join [FILE]', 'concatenate the source CoffeeScript before compiling'], ['-m', '--map', 'generate source map and save as .js.map files'], ['-M', '--inline-map', 'generate source map and include it directly in output'], ['-l', '--literate', 'treat stdio as literate style coffeescript'], ['-n', '--nodes', 'print out the parse tree that the parser produces'], ['--nodejs [ARGS]', 'pass options directly to the "node" binary'], ['--no-header', 'suppress the "Generated by" header'], ['-o', '--output [PATH]', 'set the output path or path/filename for compiled JavaScript'], ['--prettier', 'compile using Prettier'], ['-p', '--print', 'print out the compiled JavaScript'], ['-r', '--require [MODULE*]', 'require the given module before eval or REPL'], ['-s', '--stdio', 'listen for and compile scripts over stdio'], ['-t', '--transpile', 'pipe generated JavaScript through Babel'], ['--tokens', 'print out the tokens that the lexer/rewriter produce'], ['-v', '--version', 'display the version number'], ['-w', '--watch', 'watch scripts for changes and rerun commands']];
  ...
  exports.run = function() {
    ...
    if (opts.output) {
      ...
      if (indexOf.call(outputBasename, '.') >= 0 && (outputBasename !== '.' && outputBasename !== '..') && !ends(opts.output, path.sep)) {
  ...
  compileScript = function(file, input, base = null) {
    ...
    try {
      ...
      if (opts.tokens) {
        ...
        return console.log(util.inspect(CoffeeScript.babylon(task.input, task.options), false, null));
      } else if (opts.ast) {
        ...
        if (isLiterate(file)) {
          task.input = invertLiterate(task.input);
        }
  ...
  compileOptions = function(filename, base) {
    ...
    answer = {
      filename: filename,
      literate: opts.literate || isLiterate(filename),
      bare: opts.bare,
