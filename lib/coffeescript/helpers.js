...
(function() {
  ...
  var UNICODE_CODE_POINT_ESCAPE, attachCommentsToNode, buildLocationData, buildLocationHash, buildTokenDataDictionary, dump, expandLocationDataToInclude, extend, flatten, getNumberValue, isBoolean, isFunction, isNumber, isPlainObject, isString, isToken, locationFields, mergeAstLocationData, parseNumber, ref, repeat, syntaxErrorToString, traverseBabylonAst, traverseBabylonAsts, unicodeCodePointToUnicodeEscapes,
    splice = [].splice,
    indexOf = [].indexOf,
  ...
  // another AST node.
  exports.mergeAstLocationData = mergeAstLocationData = function(intoNode, fromNode, {justLeading, justEnding} = {}) {
  ...
  exports.parseNumber = parseNumber = function(str) {
    var base;
    base = (function() {
      switch (str.charAt(1)) {
        case 'b':
  ...
  exports.makeDelimitedLiteral = function(body, options = {}) {
    ...
    regex = options.justEscapeDelimiter ? RegExp(`\\\\?(${options.delimiter // (Possibly escaped) delimiter.
})`, "g") : RegExp(`(\\\\\\\\)|(\\\\0(?=[1-7]))|\\\\?(${options.delimiter // Escaped backslash.
