...
(function() {
  ...
  ({count, repeat, invertLiterate, merge, dump, makeDelimitedLiteral, attachCommentsToNode, throwSyntaxError, replaceUnicodeCodePointEscapes, getNumberValue} = require('./helpers'));
  ...
  exports.Lexer = Lexer = class Lexer {
    ...
    tokenize(code, opts = {}) {
      ...
      this.comments = (ref = opts.comments) != null ? ref : [];
      ...
      if (end = this.ends.pop()) {
        this.error(`missing ${end.tag}`, ((ref1 = end.origin) != null ? ref1 : end)[2]);
      }
      if (opts.rewrite === false) {
        return this.tokens;
      }
      return {
        tokens: (new Rewriter).rewrite({tokens: this.tokens, comments: this.comments}),
    ...
    commentToken(chunk = this.chunk, {offsetInChunk = 0, nonInitial, dontShift, heregex} = {}) {
      var commentAttachment, commentAttachments, content, contents, getIndentSize, hasSeenFirstCommentLine, here, hereLeadingWhitespace, hereTrailingWhitespace, i, indent, indentSize, leadingNewLine, leadingNewlineOffset, leadingNewlines, leadingWhitespace, length, match, matchIllegal, noIndent, nonHere, placeholderToken, precedingNonCommentLines, prev, withLeadingWhitespace;
      ...
      if (here != null) {
        ...
        contents = [
          {
            ...
            indent: this.indent // TODO: should even bother including this? not using since it's unreliable eg at the beginning of a new block
          }
        ];
      } else {
        ...
        contents = content.split('\n').map(function(line, index) {
          ...
          ret = {
            ...
            content,
            leadingWhitespace: `${precedingNonCommentLines}${!hasSeenFirstCommentLine ? leadingNewlines : ''}${leadingWhitespace}`
          };
      ...
      commentAttachments = (function() {
        ...
        for (i = j = 0, len = contents.length; j < len; i = ++j) {
          ({content, length, leadingWhitespace, indent} = contents[i]);
          if (i !== 0) {
            nonInitial = true;
          }
          ...
          if (dontShift) {
            commentAttachment.dontShift = true;
          }
          if (heregex) {
      ...
      this.comments.push(...commentAttachments);
      prev = this.prev();
      if (!prev) {
    ...
    regexToken() {
      var body, closed, comment, comments, delimiter, end, flags, index, j, len, match, opts, origin, prev, ref, ref1, regex, tokens;
      switch (false) {
        ...
        case !(match = this.matchWithInterpolations(HEREGEX, '///')):
          ({tokens, index} = match);
          comments = (() => {
            var comment, commentIndex, leadingWhitespace, matchedComment, nonInitial, withLeadingWhitespace;
            nonInitial = false;
            comments = [];
    ...
    outdentToken({moveOut, noNewlines, outdentLength = 0, offset = 0, indentSize}) {
      ...
      while (moveOut > 0) {
        ...
        if (!lastIndent) {
          ...
          moveOut -= dent;
          this.tokens[this.tokens.length - 1].indentSize = indentSize + moveOut;
        }
      }
      if (dent) {
        this.outdebt -= moveOut;
    ...
    csxToken() {
      var afterTag, csxTag, end, endToken, firstChar, fullId, id, input, j, len, match, offset, openingTagToken, prev, prevChar, prop, properties, ref, tagToken, token, tokens;
      ...
      if (firstChar === '<') {
        ...
        for (j = 0, len = properties.length; j < len; j++) {
          ...
          this.token('.', '.', {offset});
          offset += 1;
          this.token('PROPERTY', prop, {offset});
          offset += prop.length;
        }
        this.token('CALL_START', '(', {
          generated: true,
          offset: 1,
          length: id.length // encode the opening tagname location
        });
        this.token('[', '[', {
          generated: true
        });
        ...
        if (this.chunk.slice(0, 2) === '/>') { // self-closing tag
          ...
          endToken = this.token('CALL_END', ')', {
            ...
            data: {
              ...
              closingTagNameToken: this.makeToken('IDENTIFIER', match[1], {
                offset: end
              }),
              closingTagClosingBracketToken: this.makeToken('>', '>', {
                offset: end + match[1].length
              })
            }
          });
    ...
    matchWithInterpolations(regex, delimiter, closingDelimiter = delimiter, interpolators = /^#\{/) {
      var braceInterpolator, close, column, firstToken, hasSeenInterpolation, index, interpolationOffset, interpolator, lastToken, line, match, nested, offset, offsetInChunk, open, ref, ref1, rest, str, strPart, tokens;
      tokens = [];
      ...
      while (true) {
        ...
        tokens.push(this.makeToken('NEOSTRING', strPart, {
          ...
          generated: strPart.length === 0
        }));
        str = str.slice(strPart.length);
        offsetInChunk += strPart.length;
        if (!(match = interpolators.exec(str))) {
          if (strPart.length === 0 && !hasSeenInterpolation) {
            tokens[tokens.length - 1].generated = false;
          }
          break;
        }
        [interpolator] = match;
    ...
    mergeInterpolationTokens(tokens, options, fn) {
      var $, comment, converted, csx, double, endOffset, firstIndex, heregex, i, indent, j, k, lastToken, len, len1, locationToken, lparen, placeholderToken, quote, ref, ref1, rparen, tag, token, tokensToPush, val, value;
          ...
          case 'TOKENS':
            ...
            if (value.length === 2 && (value[0].comments || value[1].comments)) {
              ...
              for (k = 0, len1 = value.length; k < len1; k++) {
                ...
                placeholderToken.comments.push(...((function() {
                  var l, len2, ref1, results;
                  ref1 = val.comments;
                  results = [];
                  for (l = 0, len2 = ref1.length; l < len2; l++) {
                    comment = ref1[l];
                    comment.forceLeading = true;
    ...
    getLocationDataCompensation(start, end) {
      var compensation, index, length, ref;
      compensation = 0;
    ...
    getLineAndColumnFromChunk(offset) {
      var column, compensation, lastLine, lineCount, ref, string;
      if (offset === 0) {
      ...
      return [this.chunkLine + lineCount, column, this.chunkOffset + offset + compensation];
    }
    ...
    token(tag, value, {offset, length, origin, data, generated} = {}) {
      var token;
      token = this.makeToken(tag, value, {offset, length, origin, data, generated});
