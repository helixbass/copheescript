...
(function() {
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, getNumberValue} = require('./helpers'));
  ...
  exports.Base = Base = (function() {
    class Base {
      ...
      compileToBabylon(o, level) {
        ...
        return this._compileToBabylon(o);
      }

      compileClosureToBabylon(o) {
        return new Call(new Code([], Block.wrap([this])), []).compileToBabylon(o);
      }
      ...
      makeReturn(res) {
        ...
        me = this.unwrapAll();
        if (res) {
          return new Call(new Value(new IdentifierLiteral(res), [new Access(new PropertyName('push'))]), [me]);
        } else {
        ...
        return (compiled) => {
          ...
          return [
            ...
            ...(this.returns ? [
              {
                type: 'ExpressionStatement',
                expression: new Assign(resultsVar,
              new Arr()).compileToBabylon(o)
              }
            ] : []),
            compiled,
            ...(this.returns ? [new Return(resultsVar).compileToBabylon(o)] : [])
          ];
        };
      }
  ...
  exports.For = For = (function() {
    class For extends While {
      ...
      _compileToBabylon(o) {
        ...
        indexVar = this.object && index || new IdentifierLiteral(scope.freeVariable('i', {
          single: true
        }));
        ...
        if (!this.range) {
          if (name) {
            this.body.expressions.unshift(new Assign(name, new Value(sourceVar, [new Index(keyVar)])));
          }
        }
        if (this.object) {
      ...
      compileForPartsToBabylon({o, keyVar, indexVar, sourceVar, stepVar}) {
        ...
        return {
          init: {
            ...
            expressions: [wrapInAssignToKeyVar(new Assign(indexVar, new NumberLiteral('0'))).compileToBabylon(o), new Assign(lengthVar, new Value(sourceVar, [new Access(new PropertyName('length'))])).compileToBabylon(o)]
          },
          test: new Op('<', indexVar, lengthVar).compileToBabylon(o),
          update: wrapInAssignToKeyVar(this.step ? new Op('+=', indexVar, stepVar) : new Op('++', indexVar, null, !shouldWrapInAssignToKeyVar)).compileToBabylon(o)
        };
      }
