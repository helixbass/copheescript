...
// multiline need to be indented.
  ...
  exports.For = For = (function() {
    class For extends While {
      ...
      _compileToBabylon(o) {
        var indexVar, lengthVar, name, scope, sourceVar;
        ({scope} = o);
        ...
        if (this.object) {
          return this.compileObjectToBabylon({o, name, sourceVar});
        }
        indexVar = new IdentifierLiteral((this.object && index) || scope.freeVariable('i', {
          single: true
        }));
        lengthVar = new IdentifierLiteral(scope.freeVariable('len')); // unless @step and stepNum? and down
        this.body.expressions.unshift(new Assign(name, new Value(sourceVar, [new Index(indexVar)])));
        return {
          type: 'ForStatement',
          init: {
            // TODO: do we have an AST type for a sequence?
            type: 'SequenceExpression',
            expressions: [new Assign(indexVar, new NumberLiteral('0')).compileToBabylon(o), new Assign(lengthVar, new Value(sourceVar, [new Access(new PropertyName('length'))])).compileToBabylon(o)]
          },
