  ...
  exports.StringLiteral = StringLiteral = class StringLiteral extends Literal {
    constructor(originalValue, {
      ...
      if (this.heregex) {
        ...
        val = this.removeEscapedNewlines(val);
        val = !this.fromSourceString ? val : heredoc ? (this.indent ? indentRegex = RegExp(`\\n${this.indent}`, "g") : void 0, indentRegex ? val = val.replace(indentRegex, '\n') : void 0, this.initialChunk ? val = val.replace(LEADING_BLANK_LINE, '') : void 0, this.finalChunk ? val = val.replace(TRAILING_BLANK_LINE, '') : void 0, val) : this.collapseNewlines(val);
      }
    ...
    removeEscapedNewlines(value) {
      return value.replace(STRING_ESCAPED_NEWLINE, '$1');
    }

    collapseNewlines(value) {
      return value.replace(STRING_COLLAPSIBLE_NEWLINE, (match, offset) => {
    ...
    astProperties() {
      return {
        value: processEscapes(this.collapseNewlines(this.removeEscapedNewlines(this.originalValue))),
        extra: {
...
// multiline need to be indented.
  ...
  STRING_ESCAPES = /((?:\\\\)+)|(\\u{[\da-fA-F]+})|(\\u[\da-fA-F]{4})|(\\x[\da-fA-F]{2})|(\\[^\S\n]*\n\s*)|(\\0(?=\d))|(\\.)/g; // Consume (and preserve) an even number of backslashes.
  // Remove escaped newlines.
  // Null character mistaken as octal escape.

  processEscapes = function(string) {
    return string.replace(STRING_ESCAPES, function(match, doubleBackslashes, unicodeCodePointEscape, unicodeEscape, hexEscape, escapedNewline, nul, escape) {
      var escapedCharacter;
      if (doubleBackslashes) {
        return '\\';
      }
      if (unicodeCodePointEscape) {
        return String.fromCodePoint(parseInt(unicodeCodePointEscape.slice(3, +(unicodeCodePointEscape.length - 2) + 1 || 9e9), 16));
      }
      if (unicodeEscape) {
        return String.fromCodePoint(parseInt(unicodeEscape.slice(2, 7), 16));
      }
      if (hexEscape) {
        return String.fromCodePoint(parseInt(hexEscape.slice(2, 5), 16));
      }
      if (escapedNewline) {
        return '';
      }
