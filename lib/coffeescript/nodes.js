...
(function() {
  ...
  var Access, Arr, Assign, AwaitReturn, BABYLON_STATEMENT_TYPES, Base, Block, BooleanLiteral, CSXTag, Call, Class, ClassProperty, Code, CodeFragment, ComputedPropertyName, DefaultLiteral, Directive, Elision, ExecutableClassBody, Existence, Expansion, ExportAllDeclaration, ExportDeclaration, ExportDefaultDeclaration, ExportNamedDeclaration, ExportSpecifier, ExportSpecifierList, Extends, For, FuncGlyph, HEREGEX_OMIT, HereComment, HoistTarget, IdentifierLiteral, If, ImportClause, ImportDeclaration, ImportDefaultSpecifier, ImportNamespaceSpecifier, ImportSpecifier, ImportSpecifierList, In, Index, InfinityLiteral, Interpolation, JS_FORBIDDEN, LEADING_BLANK_LINE, LEVEL_ACCESS, LEVEL_COND, LEVEL_LIST, LEVEL_OP, LEVEL_PAREN, LEVEL_TOP, LineComment, Literal, ModuleDeclaration, ModuleSpecifier, ModuleSpecifierList, NEGATE, NO, NaNLiteral, NullLiteral, NumberLiteral, Obj, Op, Param, Parens, PassthroughLiteral, PropertyName, Range, RegexLiteral, RegexWithInterpolations, Return, SIMPLENUM, SIMPLE_STRING_OMIT, STRING_OMIT, Scope, Sequence, Slice, Splat, StatementLiteral, StringLiteral, StringWithInterpolations, Super, SuperCall, SuperLiteral, Switch, SwitchWhen, TAB, THIS, TRAILING_BLANK_LINE, TaggedTemplateCall, ThisLiteral, Throw, Try, UNICODE_CODE_POINT_ESCAPE, UNICODE_ESCAPE, UTILITIES, UTILITIES_BABYLON, UndefinedLiteral, Value, While, YES, YieldReturn, addDataToNode, assignEmptyTrailingLocationData, astAsBlock, astLocationFields, attachCommentsToNode, babylon, codePointToString, compact, del, dump, emptyLocationData, ends, extend, flatten, fragmentsToText, getNumberValue, greater, hasLineComments, indentInitial, isBoolean, isFunction, isLiteralArguments, isLiteralThis, isNumber, isPlainObject, isUnassignable, lesser, lesserAstLoc, lesserEndLoc, lesserStartLoc, locationDataToAst, locationDataToString, makeDelimitedLiteral, mapValues, merge, mergeAstLocationData, mergeLocationData, moveComments, multident, prettier, replaceUnicodeCodePointEscapes, shouldCacheOrIsAssignable, some, starts, throwSyntaxError, traverseBabylonAst, unfoldSoak, unicodeEscapesToString, unshiftAfterComments, utility, utilityBabylon,
    indexOf = [].indexOf,
    splice = [].splice,
    slice1 = [].slice;
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, replaceUnicodeCodePointEscapes, locationDataToAst, astLocationFields, isFunction, isPlainObject, isBoolean, isNumber, dump, mapValues, traverseBabylonAst, assignEmptyTrailingLocationData, UNICODE_CODE_POINT_ESCAPE} = require('./helpers'));
    ...
    class Base {
      ...
      compileToBabylon(o, level) {
        ...
        return this.withBabylonComments(o, (() => {
          ...
          return this.withAstLocationData(node._compileToBabylon(o));
        })());
      }
...
// compact(
      ...
      withEmptyBabylonLocationData(compiled) {
        return this.withAstLocationData(compiled, emptyLocationData);
      }
      ...
      withCopiedBabylonLocationData(compiled, other) {
        var field, j, len1;
        for (j = 0, len1 = astLocationFields.length; j < len1; j++) {
          field = astLocationFields[j];
      ...
      withEmptyTrailingLocationDataFrom(other) {
        return assignEmptyTrailingLocationData(this, other);
      }

      mergeLocationDataFrom(node) {
        this.locationData = this.locationData == null ? node.locationData : mergeLocationData(this.locationData, node.locationData);
        return this;
      }
      ...
      sniffDirectives(expressions, {replace, notFinalExpression} = {}) {
        var directives, expr, index, lastIndex;
        directives = [];
        index = 0;
      ...
      makeReturn({accumulator, mark} = {}) {
        var me;
        if (mark) {
          return this.canBeReturned = true;
        }
        me = this.unwrapAll();
        return this.withLocationData(accumulator ? new Call(new Value(new IdentifierLiteral(accumulator), [new Access(new PropertyName('push'))]), [me]) : new Return(me));
      }
      ...
      ast(o, level) {
        ...
        if (this.isStatement(o) && o.level !== LEVEL_TOP && o.scope) {
          this.makeReturn({
            mark: true
          });
        }
      ...
      withAstLocationData(ast, node, {force} = {}) {
        ...
        if (node && 'force' in node && !('locationData' in node)) {
          force = node.force;
          node = null;
        }
        ...
        if (!(locationData && ast && ((ast.start == null) || force))) {
          return ast;
        }
    ...
    class Block extends Base {
      ...
      makeReturn(opts) {
        ...
        if (opts != null ? opts.mark : void 0) {
          return (ref2 = this.expressions[len - 1]) != null ? ref2.makeReturn(opts) : void 0;
        }
        while (len--) {
      ...
      prettier(o) {
        ...
        ast = this.compileToBabylon(merge(o, {
          forPrettier: true
        }));
      ...
      compileRootToBabylon(o) {
        ...
        compiledBody = HoistTarget.expandBabylon(this.compileWithDeclarationsToBabylon(merge(o, {
          root: true
        })));
        if (compiledBody.length) {
          compiledBody = this.hoistBabylonComments(compiledBody);
        }
        ...
        program = this.includeCommentsInLocationData(this.withAstLocationData({
          type: 'Program',
          sourceType: 'module',
          body: compiledBody,
      ...
      _compileToBabylon(o) {
        ...
        if (!top && !root) {
          ...
          return this.wrapInParensIf(o.level >= LEVEL_LIST)(new Sequence(this.expressions).withLocationDataFrom(this)).compileToBabylon(o);
        }
        this.directives = [];
        if (withDeclarations) {
          this.sniffDirectives(this.expressions, {
            replace: true
          });
        }
        if (root && !o.bare) {
        ...
        return this.includeCommentsInLocationData(this.withAstLocationData(this.withAstType({body, directives: this.directives})));
      }
      ...
      hoistBabylonComments(compiled, {onlyTrailing, onlyLeading} = {}) {
        ...
        targetNode = Array.isArray(compiled) ? (ref1 = (function() {
          var item, j, len1;
          for (j = 0, len1 = compiled.length; j < len1; j++) {
            item = compiled[j];
            if (item.type) {
      ...
      asExpressionStatement(compiled) {
        return this.includeCommentsInLocationData(this.hoistBabylonComments((function() {
          ...
          if ((ref1 = compiled.type) === 'VariableDeclaration' || ref1 === 'ImportDeclaration' || ref1 === 'ClassMethod' || ref1 === 'ClassProperty') {
            return compiled;
          }
          return {
      ...
      astFull(o) {
        ...
        if (o.level !== LEVEL_TOP && this.expressions.length) {
      ...
      rootToAst(o) {
        var body, comment, comments, j, len1, program, programLocationData;
        this.initializeScope(o);
        body = this.bodyToAst(merge(o, {
          level: LEVEL_TOP,
          root: true
        }));
        comments = this.commentsToAst();
      ...
      commentsToAst() {
        var content, here, heregex, indent, j, len1, locationData, ref1, ref2, results;
        if (!((ref1 = this.allComments) != null ? ref1.length : void 0)) {
          return [];
        }
        ref2 = this.allComments;
        results = [];
        for (j = 0, len1 = ref2.length; j < len1; j++) {
          ({here, content, locationData, heregex, indent} = ref2[j]);
          if (!heregex) {
            results.push({
              type: here ? 'CommentBlock' : 'CommentLine',
              value: content,
      ...
      bodyToAst(o) {
        var checkForDirectives, node, root;
        root = del(o, 'root');
        checkForDirectives = del(o, 'checkForDirectives');
        this.directives = [];
        if (root || checkForDirectives) {
          this.sniffDirectives(this.expressions, {
            replace: true,
          ...
          for (j = 0, len1 = ref1.length; j < len1; j++) {
            ...
            results.push((() => {
              ...
              if (node instanceof Block || ast.type === 'BlockStatement') {
                return this.extractDirectives(ast.body, o);
              }
              if (node.isStatement(o) || node instanceof Class) {
                return ast;
              }
              ast = this.extractDirectives(ast, o);
              if (!ast) {
                return [];
              }
              if (!Array.isArray(ast)) {
  ...
  exports.StringLiteral = StringLiteral = class StringLiteral extends Literal {
    constructor(originalValue, {
      ...
      this.value = this.delimit(val);
    }

    delimit(val, opts = {}) {
      return makeDelimitedLiteral(val, merge({
    ...
    withoutQuotesInLocationData() {
      ...
      this.locationData = merge({}, this.locationData);
      this.locationData.first_column += this.quote.length;
      if (endsWithNewline) {
        this.locationData.last_line -= 1;
        this.locationData.last_column = this.locationData.last_line === this.locationData.first_line ? this.locationData.first_column + this.originalValue.length - '\n'.length : this.originalValue.length - '\n'.length - this.originalValue.lastIndexOf('\n');
      } else {
        this.locationData.last_column -= this.quote.length;
      }
      this.locationData.range = [this.locationData.range[0] + this.quote.length, this.locationData.range[1] - this.quote.length];
      return this;
    }
    ...
    astFull(o) {
      ...
      if (!o.compiling && this.quote.length === 3 && this.originalValue.indexOf('\n') > -1) {
    ...
    astProperties(o) {
      return {
        value: o.compiling ? this.unquote() : unicodeEscapesToString(this.originalValue),
        extra: {
          raw: o.compiling ? this.value : this.delimit(this.originalValue, {
            justEscapeDelimiter: true
          })
        }
      };
    }
    ...
    CSXTextToAst(o) {
      ...
      return Object.assign({
        type: 'JSXText',
        value: o.compiling ? unquoted : this.originalValue,
        extra: {
          raw: o.compiling ? unquoted : this.originalValue
        }
  ...
  exports.PassthroughLiteral = PassthroughLiteral = class PassthroughLiteral extends Literal {
    ...
    _compileToBabylon(o) {
      ...
      if (o.forPrettier) {
        return this.withEnhancedComments(this.parsedJS());
      }
    ...
    astProperties(o) {
      return {
        value: o.compiling ? this.value : this.originalValue,
        here: this.here
      };
    }
  ...
  exports.ThisLiteral = ThisLiteral = class ThisLiteral extends Literal {
    ...
    astFull(o) {
      ...
      return Object.assign(value === 'this' || !compiling ? {
        type: 'ThisExpression',
        ...(compiling ? {} : {shorthand: this.shorthand})
      } : {
        type: 'Identifier', // TODO: refine/share code?
        name: value
      }, this.astLocationData(), this.astReturns());
    }
  ...
  exports.SuperLiteral = SuperLiteral = class SuperLiteral extends Literal {
    astType() {
      return 'Super';
    }
...
// multiline need to be indented.
  ...
  exports.YieldReturn = YieldReturn = class YieldReturn extends Return {
    ...
    astFull(o) {
      ...
      return new Op('yield', new Return(this.expression)).withLocationDataFrom(this).ast(o);
    }
  ...
  exports.AwaitReturn = AwaitReturn = class AwaitReturn extends Return {
    ...
    astFull(o) {
      ...
      return new Op('await', new Return(this.expression)).withLocationDataFrom(this).ast(o);
    }
  ...
  exports.Value = Value = (function() {
    class Value extends Base {
      ...
      looksStatic(className) {
        var byClassName, ref1;
        if (!((this.this || this.base instanceof ThisLiteral || (byClassName = this.base.value === className)) && this.properties.length === 1 && ((ref1 = this.properties[0].name) != null ? ref1.value : void 0) !== 'prototype')) {
          return false;
        }
        if (byClassName) {
          return {className};
      ...
      object() {
        ...
        object.canBeReturned = this.canBeReturned;
        return object;
      }
      ...
      astFull(o) {
        ...
        if (o.compiling) {
          ...
          if (property instanceof Slice) {
            return property.compileValueToBabylon(o, this.object().ast(o, LEVEL_ACCESS));
          }
        }
  ...
  exports.Call = Call = (function() {
    class Call extends Base {
      ...
      CSXToAst(o) {
        var attr, attrProps, attributes, attributesAst, base1, closingElement, closingElementLocationData, closingFragment, closingFragmentLocationData, compiled, compiledAttr, content, currentExpr, obj, openingElement, openingElementLocationData, openingFragment, openingFragmentLocationData, prop, rangeDiff, shiftAstLocationData, tagName;
        ...
        return {
          ...((function() {
            ...
            if (!tagName.value.length) {
              ...
              closingFragmentLocationData = mergeAstLocationData(locationDataToAst(tagName.closingTagOpeningBracketLocationData), locationDataToAst(tagName.closingTagClosingBracketLocationData));
              ...
              return Object.assign({
                ...
                openingFragment,
                closingFragment
              }, mergeAstLocationData(openingFragmentLocationData, closingFragmentLocationData), this.astReturns());
            } else {
              ...
              attributesAst = flatten((function() {
                var k, l, len2, len3, ref2, results;
                if (attributes.base instanceof Arr) {
                  ...
                  for (k = 0, len2 = ref2.length; k < len2; k++) {
                    ...
                    if (Array.isArray(compiled)) {
                      for (l = 0, len3 = compiled.length; l < len3; l++) {
                        compiledAttr = compiled[l];
              ...
              openingElement = Object.assign({
                ...
                name: this.variable.unwrap().ast(o), //, LEVEL_ACCESS
                selfClosing: !content,
              ...
              if (content) {
                ...
                closingElement = Object.assign({
                  ...
                  name: this.withCopiedBabylonLocationData(this.variable.unwrap().ast(o), locationDataToAst(tagName.closingTagNameLocationData)) //, LEVEL_ACCESS
              ...
              return Object.assign({
                ...
                openingElement,
                closingElement
              }, closingElement != null ? mergeAstLocationData(openingElementLocationData, closingElementLocationData) : openingElementLocationData, this.astReturns());
            }
          }).call(this)),
          children: content && !(typeof (base1 = content.base).isEmpty === "function" ? base1.isEmpty() : void 0) ? (content.base.csx = true, compact(flatten([content.ast(o)]))) : [] //, LEVEL_LIST
        };
      }
  ...
  exports.Super = Super = (function() {
    class Super extends Base {
      ...
      astFull(o) {
        ...
        compiled = new Value(new SuperLiteral, this.accessor ? [this.accessor] : []).withLocationDataFrom(this).ast(o);
        if (salvagedComments) {
          attachCommentsToNode(salvagedComments, this.accessor.name);
        }
        return compiled;
  ...
  exports.Slice = Slice = (function() {
    class Slice extends Base {
      ...
      compileValueToBabylon(o, compiledValue) {
        ...
        return this.withAstLocationData({
          type: 'CallExpression',
          callee: {
  ...
  exports.Obj = Obj = (function() {
    class Obj extends Base {
      constructor(properties1 = [], {generated: generated = false, isClassBody} = {}) {
        super();
        this.properties = properties1;
        this.generated = generated;
        this.isClassBody = isClassBody;
          ...
          results.push((() => {
            ...
            if (key instanceof Value && key.hasProperties()) {
              ...
              if (compiling) {
                ...
                return new Assign(key, prop, {
                  context: 'object',
                  shorthand: true
                }).withLocationDataFrom(prop);
              }
            }
            if (key !== prop) {
              return prop;
            }
            return prop.withLocationData(prop.shouldCache() && compiling ? ([key, value] = prop.base.cache(o), key instanceof IdentifierLiteral ? key = key.withLocationData(new PropertyName(key.value)) : void 0, new Assign(key, value, {
              context: 'object'
            // `{ [foo()] }` output as `{ [ref = foo()]: ref }`.
            })) : prop instanceof Value && prop.base instanceof ComputedPropertyName && compiling ? prop.base.value.shouldCache() ? ([key, value] = prop.base.value.cache(o), key instanceof IdentifierLiteral ? key = key.withLocationData(new Value(new ComputedPropertyName(key.value))) : void 0, new Assign(key, value, {
              context: 'object'
      ...
      CSXAttributesToAst(o) {
        var compiledProps, prop;
        compiledProps = (function() {
          var j, len1, ref1, results;
          ref1 = this.properties;
          results = [];
          for (j = 0, len1 = ref1.length; j < len1; j++) {
            prop = ref1[j];
            prop.csx = true;
            results.push(prop.ast(o)); //, LEVEL_TOP
          }
          return results;
        }).call(this);
        if (this.properties.length === 1 && this.properties[0] instanceof Splat) {
          ...
          return this.withAstLocationData(compiledProps[0], {
            force: true
          });
        }
        ...
        return {
          properties: (function() {
            ...
            for (j = 0, len1 = ref1.length; j < len1; j++) {
              prop = ref1[j];
              results.push((() => {
                var compiledKey, compiledValue, isAssignmentPattern, isComputedPropertyName, shorthand, value, variable;
                if (prop instanceof Splat) {
                  return prop.ast(o);
                }
                ...
                compiledKey = (isComputedPropertyName ? variable.base.value : variable.unwrap()).ast(o, LEVEL_LIST);
                isAssignmentPattern = prop instanceof Assign && prop.context !== 'object'; // TODO: restructure this Assign in expandProperties() to avoid this special case?
                // key = merge {}, compiledKey
                // key.declaration = no if key.declaration
                compiledValue = value.ast(o, LEVEL_LIST);
                return prop.withAstLocationData({
                  type: 'ObjectProperty',
                  key: compiledKey,
                  value: isAssignmentPattern ? {
                  ...
                  computed: (() => {
                    ...
                    if (!o.compiling) { // and variable.unwrap() instanceof StringWithInterpolations
                      return false;
                    }
                    return variable.shouldCache();
                  })()
                });
              })());
            }
  ...
  exports.Arr = Arr = (function() {
    class Arr extends Base {
      constructor(objs, {lhs: lhs1 = false} = {}) {
        super();
        this.lhs = lhs1;
        this.objects = objs || [];
        this.propagateLhs();
  ...
  exports.Class = Class = (function() {
    class Class extends Base {
      ...
      fixBodyLocationData() {
        ...
        return this.body.withEmptyTrailingLocationDataFrom(this);
      }
      ...
      walkBody(o) {
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          if (expression instanceof Value && expression.isObject(true)) {
            ...
            start = 0;
            pushSlice = function() {
              var k, len2, obj, prop, ref2;
              if (end > start) {
                obj = new Obj(properties.slice(start, end), {
                  generated: true,
                  isClassBody: true
                });
                ref2 = properties.slice(start, end);
                for (k = 0, len2 = ref2.length; k < len2; k++) {
                  prop = ref2[k];
                  obj.mergeLocationDataFrom(prop);
                }
                obj = obj.withLocationData(new Value(obj));
                return exprs.push(obj);
              }
            };
      ...
      addClassProperty(node) {
        var operatorToken, value, variable;
        ({variable, value, operatorToken} = node);
        return node.withLocationData(new ClassProperty({
          name: variable.properties[0],
          isStatic: variable.looksStatic(this.name), // is always static, this is to get className
          value,
          operatorToken
        }));
      }
      ...
      addInitializerMethod({
        ...
        var base1, isConstructor, methodName;
      ...
      astFull(o) {
        var ref1;
        this.declareName(o);
        this.fixBodyLocationData();
        this.name = this.determineName();
        this.body.isClassBody = true;
        this.walkBody(o);
        this.sniffDirectives(this.body.expressions, {
          replace: true
        });
      ...
      astProperties(o) {
        ...
        return Object.assign({
          ...
          computed: (() => {
            ...
            if (this.name instanceof Access && this.name.name instanceof ComputedPropertyName) {
              return true;
            }
            return false;
          })(),
          operator: (ref1 = (ref2 = this.operatorToken) != null ? ref2.value : void 0) != null ? ref1 : '='
        }, ((ref3 = this.isStatic) != null ? ref3.className : void 0) ? {
          staticClassName: this.isStatic.className
        } : void 0);
      }
  ...
  exports.ExecutableClassBody = ExecutableClassBody = (function() {
    class ExecutableClassBody extends Base {
      ...
      _compileToBabylon(o) {
        ...
        this.body.expressions.unshift(...((function() {
          ...
          for (j = 0, len1 = directives.length; j < len1; j++) {
            ...
            results.push(new Directive(directive).withLocationDataFrom(directive));
          }
          return results;
        })()));
      ...
      walkBody() {
        var directives;
        directives = this.sniffDirectives(this.body.expressions);
  ...
  exports.ImportSpecifier = ImportSpecifier = class ImportSpecifier extends ModuleSpecifier {
    ...
    astProperties(o) {
      ...
      compiledOriginal = this.original.ast(o);
      return {
        imported: compiledOriginal,
        local: (ref1 = (ref2 = this.alias) != null ? ref2.ast(o) : void 0) != null ? ref1 : compiledOriginal
      };
    }
  ...
  exports.ImportNamespaceSpecifier = ImportNamespaceSpecifier = class ImportNamespaceSpecifier extends ImportSpecifier {
    astProperties(o) {
      return {
        local: (() => {
          if (this.alias) {
            return this.alias.ast(o);
          }
          return this.original.ast(o);
        })()
      };
    }
  ...
  exports.ExportSpecifier = ExportSpecifier = class ExportSpecifier extends ModuleSpecifier {
    ...
    astProperties(o) {
      ...
      return {
        ...
        exported: (ref1 = (ref2 = this.alias) != null ? ref2.ast(o) : void 0) != null ? ref1 : compiledOriginal
      };
    }
  ...
  exports.Assign = Assign = (function() {
    class Assign extends Base {
      constructor(variable1, value1, options = {}) {
        super();
        this.variable = variable1;
        this.value = value1;
        ({context: this.context, param: this.param, subpattern: this.subpattern, operatorToken: this.operatorToken, moduleDeclaration: this.moduleDeclaration, shorthand: this.shorthand, originalContext: this.originalContext = this.context} = options);
        this.propagateLhs();
      }
      ...
      CSXAttributeToAst(o) {
        return Object.assign({
          type: 'JSXAttribute',
          name: this.variable.base.withAstLocationData({
            type: 'JSXIdentifier',
          ...
          value: (() => {
            ...
            return val.withAstLocationData({
      ...
      addScopeVariables(o, {checkAssignability = true} = {}) {
        ...
        return varBase.eachName((name) => {
          var alreadyDeclared, comment, commentFragments, commentsNode, j, len1, message, ref1, results;
      ...
      makeReturn(opts) {
        if ((opts != null ? opts.accumulator : void 0) || (opts != null ? opts.mark : void 0)) {
  ...
  exports.Code = Code = (function() {
    class Code extends Base {
      ...
      astFull(o) {
        this.updateOptions(o);
        this.processParams(o); // this is necessary to declare params on scope, probably could refactor to just do that
        if (this.isMethod) {
          this.mergeLocationDataFrom(this.name);
        }
      ...
      astProperties(o) {
        return {
          ...
          body: {
            ...this.body.ast(merge(o, {
              checkForDirectives: true
            }), LEVEL_TOP),
            indented: this.body.locationData.first_line > this.funcGlyph.locationData.first_line
          },
          generator: this.isGenerator,
          async: this.isAsync,
          bound: this.bound,
          ...
          results.push(((function() {
            ...
            if (splat) {
              ...
              return new Assign(name, value, {
                param: true
              }).withLocationDataFrom(name).mergeLocationDataFrom(value);
            } else {
              return name;
            }
          })()).ast(o));
      ...
      methodAstFields(o) {
        var compiledName, fields, methodScope, ref1, ref2, ref3;
        if (!this.isMethod) {
          return {};
        }
        [methodScope, o.scope] = [o.scope, o.scope.parent];
        compiledName = this.name.ast(o);
        o.scope = methodScope;
        fields = {
          kind: this.ctor ? 'constructor' : 'method',
          key: compiledName,
          computed: (() => {
            if (!o.compiling && this.name instanceof Index && this.name.index instanceof StringWithInterpolations) {
              return false;
            }
            if (this.name instanceof Index) {
              return true;
            ...
            if (this.name instanceof Access && (this.name.name instanceof NumberLiteral || this.name.name instanceof StringLiteral)) {
              return true;
            }
            return false;
          })(),
        ...
        if (!o.compiling) {
          if ((ref1 = this.isStatic) != null ? ref1.className : void 0) {
            fields.staticClassName = this.isStatic.className;
          }
          fields.operator = (ref2 = (ref3 = this.operatorToken) != null ? ref3.value : void 0) != null ? ref2 : '=';
        }
        return fields;
      }
  ...
  exports.Param = Param = (function() {
    class Param extends Base {
      ...
      astFull(o) {
        return this.name.ast(o, LEVEL_LIST);
      }
  ...
  exports.While = While = (function() {
    class While extends Base {
      ...
      processedCondition() {
        return this._processedCondition != null ? this._processedCondition : this._processedCondition = this.invert ? this.condition.invert() : this.condition;
      }
      ...
      makeReturn(opts) {
        if (opts != null ? opts.accumulator : void 0) {
          ...
          if (opts != null ? opts.mark : void 0) {
            if (this.returns) {
              return this.body.makeReturn(opts);
            }
          } else {
            return this;
          }
      ...
      compileNode(o) {
        ...
        o.indent += TAB;
        set = '';
        body = this.body.isEmpty() ? this.makeCode('') : (this.returns ? (this.body.makeReturn({
  ...
  exports.Op = Op = (function() {
    ...
    class Op extends Base {
      ...
      astProperties(o) {
        var argument, firstAst, operator, ref1, secondAst;
        ...
        switch (false) {
          ...
          case !this.isUnary():
            argument = this.isYield() || this.isAwait() ? this.first.unwrap().value !== '' ? firstAst : void 0 : firstAst;
            if (this.isAwait()) {
              return {argument};
            }
            if (this.isYield()) {
  ...
  exports.Try = Try = (function() {
    class Try extends Base {
      ...
      makeReturn(opts) {
        ...
        if (this.recovery) {
          this.recovery = this.recovery.makeReturn(opts);
        }
        return this;
      }
      ...
      astProperties(o) {
        var compiling, placeholder, ref1, ref2, ref3, ref4;
        ({compiling} = o);
        ...
        return {
          ...
          handler: compiling ? placeholder ? (this.recovery || this).withAstLocationData({
            type: 'CatchClause',
            param: placeholder.compileToBabylon(o),
            body: (this.recovery || this.withLocationData(new Block)).compileToBabylon(o, LEVEL_TOP)
          }) : void 0 : this.recovery ? this.recovery.withAstLocationData({
            type: 'CatchClause',
            param: (ref2 = (ref3 = this.errorVariable) != null ? ref3.ast(o) : void 0) != null ? ref2 : null,
            body: this.recovery.ast(o, LEVEL_TOP)
          }) : void 0,
          finalizer: (ref4 = this.ensure) != null ? ref4.ast(o, LEVEL_TOP) : void 0
        };
      }
  ...
  exports.Parens = Parens = (function() {
    class Parens extends Base {
      ...
      _compileToBabylon(o) {
        ...
        if (shouldntWrap || !compiled || Array.isArray(compiled)) {
          return compiled;
        }
        return {
          ...compiled,
  ...
  exports.StringWithInterpolations = StringWithInterpolations = (function() {
    class StringWithInterpolations extends Base {
      ...
      static fromStringLiteral(stringLiteral) {
        ...
        return new StringWithInterpolations(Block.wrap([new Value(stringLiteral.withoutQuotesInLocationData())]), {
      ...
      extractElements(o) {
        ...
        expr.traverseChildren(false, (node) => {
          var base1, comment, commentPlaceholder, empty, isEmpty, j, k, len1, len2, ref1, ref2, unwrapped;
          if (node instanceof StringLiteral) {
            ...
            isEmpty = (unwrapped = (ref1 = node.expression) != null ? ref1.unwrapAll() : void 0) instanceof PassthroughLiteral && unwrapped.generated;
          ...
          if (element instanceof StringLiteral) {
            ...
            if (!this.isNestedTag(element) || code.some((function() {
              ...
              return function({comments, isCommentBoundary}) {
                ...
                if ((comments != null ? comments.some(function(comment) {
                  return comment.here === false;
                }) : void 0) && !insideCommentBoundary) {
                  return true;
                }
      ...
      CSXContentToAst(o) {
        ...
        for (j = 0, len1 = elements.length; j < len1; j++) {
          ...
          if (element instanceof StringLiteral) {
            ...
            results.push(element.withAstLocationData({
              type: 'JSXExpressionContainer',
              expression: (function() {
  ...
  exports.Interpolation = Interpolation = (function() {
    class Interpolation extends Base {
      ...
      astFull(o) {
        if (this.expression) {
          this.error('Interpolation::ast() should only be called for empty interpolations');
        }
        return super.astFull(o);
      }
  ...
  exports.For = For = (function() {
    class For extends While {
      ...
      astProperties(o) {
        ...
        return {
          source: (ref1 = this.source) != null ? ref1.ast(o) : void 0,
          body: this.postfix ? this.body.unwrap().ast(o) : this.body.ast(o, LEVEL_TOP),
          guard: (ref2 = this.guard) != null ? ref2.ast(o) : void 0,
          name: (ref3 = this.name) != null ? ref3.ast(o) : void 0,
          index: (ref4 = this.index) != null ? ref4.ast(o) : void 0,
          step: (ref5 = this.step) != null ? ref5.ast(o) : void 0,
          postfix: this.postfix,
          own: this.own,
          await: this.await,
          style: (function() {
  ...
  exports.Switch = Switch = (function() {
    class Switch extends Base {
      ...
      astProperties(o) {
        var caseIndex, caseLocationData, compiling, consequent, consequentAst, kase, lastCaseIndex, lastTestIndex, leadingToken, ref1, ref2, ref3, test, testIndex, tests, trailingToken;
        ({compiling} = o);
        lastCaseIndex = this.cases.length - 1;
        return {
          discriminant: (ref1 = (ref2 = this.subject) != null ? ref2 : compiling ? new BooleanLiteral('false') : null) != null ? ref1.ast(o, LEVEL_PAREN) : void 0,
          cases: [
            ...flatten((function() {
              var j,
            len1,
            ...
            results1;
                  ...
                  for (testIndex = k = 0, len2 = tests.length; k < len2; testIndex = ++k) {
                    ...
                    consequentAst = (() => {
                      ...
                      return consequent.ast(o,
            LEVEL_TOP).body;
                    })();
                    caseLocationData = test.astLocationData();
                    if (consequentAst.length) {
                      caseLocationData = mergeAstLocationData(caseLocationData,
            consequentAst[consequentAst.length - 1]);
                    }
                    if (testIndex === 0) {
                      caseLocationData = mergeAstLocationData(caseLocationData,
            locationDataToAst(kase.locationData),
            {
                        justLeading: true
                      });
                    }
            ...
            locationDataToAst(kase.locationData),
            {
                    ...
                    results1.push(Object.assign({
                      type: 'SwitchCase',
                      test: (this.subject || !compiling ? test : test.invert()).ast(o,
            LEVEL_PAREN),
            ...
            ...(((ref3 = this.otherwise) != null ? ref3.expressions.length : void 0) ? [
              this.otherwise.withAstLocationData({
                type: 'SwitchCase',
                test: null,
                consequent: this.otherwise.ast(o,
              LEVEL_TOP).body
              })
            ] : [])
          ]
  ...
  exports.If = If = (function() {
    class If extends Base {
      ...
      processedCondition() {
        return this._processedCondition != null ? this._processedCondition : this._processedCondition = this.type === 'unless' ? this.condition.invert() : this.condition;
      }
      ...
      addElse(elseBody) {
        if (this.isChain) {
          ...
          this.mergeLocationDataFrom(this.elseBodyNode());
        } else {
          this.isChain = elseBody instanceof If;
          this.elseBody = this.ensureBlock(elseBody);
          if (this.elseBody.locationData && this.locationData) {
            this.mergeLocationDataFrom(this.elseBody);
          }
        }
        return this;
      }
      ...
      isStatement(o) {
        ...
        if (!o.compiling) {
          ...
          return this.bodyNode() instanceof Block || this.bodyNode().isStatement(o) || this.elseBodyNode() instanceof Block || ((ref1 = this.elseBodyNode()) != null ? ref1.isStatement(o) : void 0);
        }
        return (o != null ? o.level : void 0) === LEVEL_TOP || this.bodyNode().isStatement(o) || ((ref2 = this.elseBodyNode()) != null ? ref2.isStatement(o) : void 0);
      }
      ...
      ensureBlock(node) {
        var block;
        block = node instanceof Block ? node : new Block([node]);
        if (node.locationData) {
          return node.withLocationData(block);
        } else {
          return this.withLocationData(block);
        }
      }
      ...
      statementToAst(o) {
        ...
        return {
          test: this.useCondition(o).ast(o, LEVEL_PAREN),
          consequent: this.ensureBlock(this.body).ast(o, LEVEL_TOP),
          alternate: (() => {
            var compiled;
            if (!this.elseBody) {
              return;
            }
            compiled = this.elseBody.ast(o, LEVEL_TOP);
      ...
      expressionToAst(o) {
        return {
          test: this.useCondition(o).ast(o, LEVEL_COND),
          consequent: this.bodyNode().ast(o, o.compiling ? LEVEL_LIST : LEVEL_TOP),
          alternate: this.elseBodyNode() ? this.elseBodyNode().ast(o, o.compiling ? LEVEL_LIST : LEVEL_TOP) : o.compiling ? new UndefinedLiteral().ast(o) : void 0,
          ...this.astFields(o)
        };
      }
  ...
  astAsBlock = function(element, o) {
    var unwrapped;
    unwrapped = element.unwrap();
    if (!o.compiling && unwrapped instanceof Block && unwrapped.expressions.length > 1) {
      return unwrapped.ast(o, LEVEL_TOP);
    } else {
      return element.ast(o, LEVEL_PAREN);
    }
  };
  ...
  // taken from acorn
  codePointToString = function(code) {
    if (code <= 0xFFFF) {
      // UTF-16 Decoding
  ...
  UNICODE_ESCAPE = /(\\\\)|\\u\{([\da-fA-F]+)\}|\\u([\da-fA-F]{4})/g; // Make sure the escape isn't escaped

  unicodeEscapesToString = function(str) {
    return str.replace(UNICODE_ESCAPE, function(match, escapedBackslash, codePointHex, escapeHex) {
      if (escapedBackslash) {
        return escapedBackslash;
      }
      return codePointToString(parseInt(codePointHex != null ? codePointHex : escapeHex, 16));
    });
  };
  ...
  makeDelimitedLiteral = function(body, options = {}) {
    ...
    regex = options.justEscapeDelimiter ? RegExp(`\\\\?(${options.delimiter})`, "g") : RegExp(`(\\\\\\\\)|(\\\\0(?=[1-7]))|\\\\?(${options.delimiter // Escaped backslash.
    // Null character mistaken as octal escape.
    // (Possibly escaped) delimiter.
    // (Possibly escaped) newlines.
    // Other escapes.
})|\\\\?(?:(\\n)|(\\r)|(\\u2028)|(\\u2029))|(\\\\.)`, "g");
    body = body.replace(regex, function(...args) {
      var backslash, cr, delimiter, lf, ls, match, nul, other, ps;
  ...
  mergeLocationData = function(locationDataA, locationDataB) {
    return Object.assign(lesserStartLoc(locationDataA, locationDataB) ? {
    ...
    }, lesserEndLoc(locationDataA, locationDataB) ? {
  ...
  mergeAstLocationData = function(nodeA, nodeB, {justLeading, justEnding} = {}) {
    return {
      loc: {
        start: justEnding ? nodeA.loc.start : lesserAstLoc(nodeA.loc.start, nodeB.loc.start) ? {
        ...
        end: justLeading ? nodeA.loc.end : lesserAstLoc(nodeA.loc.end, nodeB.loc.end) ? {
  ...
  getNumberValue = function(node) {
    var val;
    node = node.unwrap();
    if (node.parsedValue != null) {
      return node.parsedValue;
    }
