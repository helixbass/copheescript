...
(function() {
  ...
  ({compact, flatten, extend, merge, del, starts, ends, some, addDataToNode, attachCommentsToNode, locationDataToString, throwSyntaxError, getNumberValue, dump} = require('./helpers'));
  ...
  exports.Base = Base = (function() {
    class Base {
      ...
      withBabylonLocationData(compiled, node) {
        var first_column, first_line, last_column, last_line, locationData, range;
        ({locationData} = node || this);
        if (!(locationData && !Array.isArray(compiled))) {
          return compiled;
        }
        ({first_line, first_column, last_line, last_column, range} = locationData);
        return merge(compiled, {
          loc: {
            start: {
              line: first_line + 1,
              column: first_column
...
// multiline need to be indented.
  ...
  exports.Range = Range = (function() {
    class Range extends Base {
      ...
      compileForToBabylon(o) {
        ...
        keyVar = del(o, 'keyVar');
        return {
          init: new Assign(keyVar, new Assign(indexVar, this.from)).compileToBabylon(o),
          test: new Op(`<${this.equals}`, indexVar, this.to).compileToBabylon(o),
          update: new Assign(keyVar, new Op('++', indexVar, null, true)).compileToBabylon(o)
        };
      }
      ...
      _compileToBabylon(o) {
        ...
        return {
          ...
          properties: (function() {
            ...
            results = [];
            for (j = 0, len1 = ref1.length; j < len1; j++) {
              ({variable, value, shorthand} = ref1[j]);
              results.push({
                type: 'ObjectProperty',
        ...
        for (j = 0, len1 = ref1.length; j < len1; j++) {
          ...
          results.push((function() {
            ...
            if (prop.shouldCache()) {
              ...
              return new Assign(key, value, 'object');
            } else if (prop instanceof Value && prop.base instanceof ComputedPropertyName) {
              ...
              if (prop.base.value.shouldCache()) {
                ...
                return new Assign(prop, prop.base.value, 'object');
              }
            } else {
              return new Assign(prop, prop, 'object', {
    ...
    class While extends Base {
        ...
        return (compiled) => {
          ...
          return [
            ...
            ...(this.returns ? [
              {
                type: 'ExpressionStatement',
                expression: this.withLocationData(new Assign(resultsVar,
      ...
      bodyWithGuard() {
        ...
        if (!(this.body.expressions.length > 1)) {
          return Block.wrap([new If(this.guard, this.body)]);
        }
        this.body.expressions.unshift(new If((new Parens(this.guard)).invert(), new StatementLiteral("continue")));
        return this.body;
      }
  ...
  exports.Op = Op = (function() {
    ...
    class Op extends Base {
      ...
      compileExistenceToBabylon(o, checkOnlyUndefined) {
        return new If(new Existence(this.first, checkOnlyUndefined), this.first, {
          type: 'if'
    ...
    class Existence extends Base {
      ...
      _compileToBabylon(o) {
        ...
        comparison = this.expression.unwrap() instanceof IdentifierLiteral && !o.scope.check(this.expression.unwrap().value) ? (() => {
          ...
          comparisonAgainstUndefined = new Op('!==', new Op('typeof', this.expression), new StringLiteral("'undefined'"));
          if (this.comparisonTarget === 'undefined') {
            return comparisonAgainstUndefined;
          }
          return new Op('&&', comparisonAgainstUndefined, new Op('!==', this.expression, new NullLiteral));
        ...
        return (o.level <= LEVEL_COND ? comparison : new Parens(comparison)).compileToBabylon(o);
      }
  ...
  exports.StringWithInterpolations = StringWithInterpolations = (function() {
    class StringWithInterpolations extends Base {
      ...
      _compileToBabylon(o) {
        var element, elements, expressions, first, index, j, last, len1, quasis, salvagedComments;
        ({elements, salvagedComments} = this.extractElementsAndComments());
        [first] = elements;
        if (!(first instanceof StringLiteral)) {
          elements.unshift(new StringLiteral(''));
        }
        [last] = slice1.call(elements, -1);
        if (!(last instanceof StringLiteral)) {
          elements.push((last = new StringLiteral('')));
        }
        quasis = [];
        expressions = [];
