...
(function() {
  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (indexOf.call(IMPLICIT_END, tag) >= 0 || (indexOf.call(CALL_CLOSERS, tag) >= 0 && newLine) || ((tag === '..' || tag === '...') && this.findTagsBackwards(i, ["INDEX_START"]))) {
            while (inImplicit()) {
              ...
              } else if (inImplicitObject() && sameLine && tag !== 'TERMINATOR' && prevTag !== ':' && !((tag === 'POST_IF' || tag === 'POST_FOR' || tag === 'POST_WHILE' || tag === 'POST_UNTIL') && startsLine && implicitObjectContinues(i + 1))) {
                endImplicitObject();
      ...
      tagPostfixConditionals() {
        ...
        return this.scanTokens(function(token, i) {
          var ref;
          if ((ref = token[0]) !== 'IF' && ref !== 'WHILE' && ref !== 'UNTIL' && ref !== 'FOR') {
            return 1;
          }
          original = token;
          this.detectEnd(i + 1, condition, action);
  ...
  IMPLICIT_CALL = ['IDENTIFIER', 'JSX_TAG', 'PROPERTY', 'NUMBER', 'INFINITY', 'NAN', 'STRING', 'STRING_START', 'REGEX', 'REGEX_START', 'JS', 'NEW', 'PARAM_START', 'CLASS', 'IF', 'TRY', 'SWITCH', 'THIS', 'WHILE', 'FOR', 'UNDEFINED', 'NULL', 'BOOL', 'UNARY', 'DO', 'DO_IIFE', 'YIELD', 'AWAIT', 'UNARY_MATH', 'SUPER', 'THROW', '@', '->', '=>', '[', '(', '{', '--', '++'];

  IMPLICIT_UNSPACED_CALL = ['+', '-'];

  // Tokens that always mark the end of an implicit call for single-liners.
  IMPLICIT_END = ['POST_IF', 'POST_FOR', 'POST_WHILE', 'POST_UNTIL', 'WHEN', 'BY', 'LOOP', 'TERMINATOR'];
