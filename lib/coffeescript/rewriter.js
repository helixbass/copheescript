...
(function() {
  ...
  var BALANCED_PAIRS, CONTROL_IN_IMPLICIT, DISCARDED, EXPRESSION_CLOSE, EXPRESSION_END, EXPRESSION_START, IMPLICIT_CALL, IMPLICIT_END, IMPLICIT_FUNC, IMPLICIT_UNSPACED_CALL, INVERSES, LINEBREAKS, Rewriter, SINGLE_CLOSERS, SINGLE_LINERS, generate, k, left, len, moveComments, right, throwSyntaxError,
    indexOf = [].indexOf;
    ...
    class Rewriter {
      ...
      addImplicitBracesAndParens() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (indexOf.call(IMPLICIT_END, tag) >= 0 || ((tag === '..' || tag === '...') && this.findTagsBackwards(i, ["INDEX_START"]))) {
            while (inImplicit()) {
      ...
      normalizeLines() {
        ...
        condition = function(token, i) {
          var ref, ref1, ref2;
          return token[1] !== ';' && (ref = token[0], indexOf.call(SINGLE_CLOSERS, ref) >= 0) && !(token[0] === 'TERMINATOR' && (ref1 = this.tag(i + 1), indexOf.call(EXPRESSION_CLOSE, ref1) >= 0)) && !(token[0] === 'ELSE' && (starter !== 'THEN' || (leading_if_then || leading_switch_when))) && !(((ref2 = token[0]) === 'CATCH' || ref2 === 'FINALLY') && (starter === '->' || starter === '=>'));
        };
        action = function(token, i) {
      ...
      tagLeadingLogical() {
        ...
        return this.scanTokens(function(token, i, tokens) {
          ...
          if (!(token[0] === 'TERMINATOR' && tokens.length >= i + 2 && (ref = (operatorToken = tokens[i + 1])[0], indexOf.call(leadingTokens, ref) >= 0))) {
            return 1;
          }
  ...
  DISCARDED = ['(', ')', '[', ']', '{', '}', '.', '?.', '::', '?::', '..', '...', ',', '=', '++', '--', '?', 'AS', 'AWAIT', 'CALL_START', 'CALL_END', 'DEFAULT', 'ELSE', 'EXTENDS', 'EXPORT', 'FORIN', 'FOROF', 'FORFROM', 'IMPORT', 'INDENT', 'INDEX_SOAK', 'LEADING_WHEN', 'OUTDENT', 'PARAM_END', 'REGEX_START', 'REGEX_END', 'RETURN', 'STRING_END', 'THROW', 'UNARY', 'DO', 'DO_IIFE', 'YIELD'].concat(IMPLICIT_UNSPACED_CALL.concat(IMPLICIT_END.concat(CONTROL_IN_IMPLICIT)));
