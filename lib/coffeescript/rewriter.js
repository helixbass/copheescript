  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      rescueStowawayComments() {
        var dontShiftBackward, dontShiftForward, insertPlaceholder, shiftCommentsBackward, shiftCommentsForward;
        ...
        dontShiftBackward = function(i, tokens) {
          var j, ref;
          j = i;
          while (j !== tokens.length && (ref = tokens[j][0], indexOf.call(DISCARDED, ref) >= 0)) {
            if (tokens[j][0] === 'REGEX_END') {
              return true;
            }
            j++;
          }
        ...
        return this.scanTokens(function(token, i, tokens) {
          var comment, dummyToken, j, ref, ref1, ret;
          if (!token.comments) {
            return 1;
          }
          ret = 1;
          if (ref = token[0], indexOf.call(DISCARDED, ref) >= 0) {
          ...
          } else if (!(token[0] === 'JS' && token.generated || dontShiftForward(i, tokens))) {
            ...
            while (j !== -1) {
              ...
              if (comment.newLine && !comment.unshift && !comment.dontShift) {
      ...
      fixIndentationLocationData() {
        return this.scanTokens(function(token, i, tokens) {
          var nextToken, nextTokenIndex, precedingComment, prevLocationData, prevToken, ref, ref1, useNextToken;
          ...
          precedingComment = this.findPrecedingComment(useNextToken ? nextToken : token, {
            indented: useNextToken, // or token[0] is 'INDENT'
            first: token[0] === 'INDENT',
      ...
      findPrecedingComment(token, {indented, first, afterPos, indentSize}) {
        ...
        matches = function(comment) {
          return (!comment.outdented || (indentSize != null) && comment.indentSize > indentSize) && comment.locationData.range[0] < tokenStart && comment.locationData.range[0] > afterPos && (indented ? comment.indented : true);
        };
        if (first) {
          lastMatching = null;
          ref = this.comments;
      ...
      exposeTokenDataToGrammar() {
        return this.scanTokens(function(token, i) {
          ...
          if (token.data && Object.keys(token.data).length || ((ref = token[0]) === 'JS' || ref === 'CALL_START' || ref === '{') && token.generated) {
