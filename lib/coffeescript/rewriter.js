  ...
  exports.Rewriter = Rewriter = (function() {
    class Rewriter {
      ...
      fixIndentationLocationData() {
        return this.scanTokens(function(token, i, tokens) {
          var precedingComment, prevLocationData, ref, useNextToken;
          ...
          useNextToken = (token.explicit || token.generated) && i !== tokens.length - 1;
          precedingComment = this.findPrecedingComment(useNextToken ? tokens[i + 1] : token, {
      ...
      findPrecedingComment(token, {indented, first, afterPos}) {
        ...
        matches = function(comment) {
          return !comment.outdented && comment.locationData.range[0] < tokenStart && comment.locationData.range[0] > afterPos && (indented ? comment.indented : true);
        };
        if (first) {
          lastMatching = null;
          ref = this.comments;
