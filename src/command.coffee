...
compileScript = (file, input, base = null) ->
  ...
  try
    ...
    if opts.tokens
      ...
      printLine(
        CoffeeScript.nodes task.input, task.options
        .toString()
        .trim(),
      )
    else if opts.ast
  ...
  compile = ->
    if opts.compile
      ...
      if generatedSourceMap
        js = "#{js}\n//# sourceMappingURL=#{helpers.baseFileName(
          sourceMapPath,
          no,
          useWinPathSep,
        )}\n"
  ...
  if opts.transpile
    ...
    unless opts.transpile.filename
      opts.transpile.filename =
        filename or path.resolve base or process.cwd(), '<anonymous>'
  else
    opts.transpile = no
  ...
  if filename
    if base
      ...
      answer = helpers.merge answer,
        ...
        generatedFile:
          helpers.baseFileName(filename, yes, useWinPathSep) + '.js'
  answer
