...
jisonLocationDataToAstLocationData = ({first_line, first_column, last_line, last_column, range, ends_line}) ->
  return
    loc:
      start:
        line:   first_line + 1
        ...
        line:   if ends_line then last_line + 2 else last_line + 1
        column: if ends_line then 0             else last_column + 1
    range: [
      range[0]
      range[1]
    ]
