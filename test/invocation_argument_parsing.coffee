...
spawnOptions =
  ...
  env:
    PATH:
      coffeeBinFolder + (if isWindows() then ';' else ':') + process.env.PATH
  shell: isWindows()
...
initialSpaceExtraArgsScript =
  require.resolve './importing/shebang_initial_space_extra_args.coffee'
...
test 'warn about non-portable shebang lines', ->
  ...
  ok(
    stderr.match(
      /^The script to be run begins with a shebang line with more than one/m,
    ),
  )
  [_, firstLine, file] =
    stderr.match /^The shebang line was: '([^']+)' in file '([^']+)'/m
  ...
  result = spawnSync(
    coffeeCommand,
    [initialSpaceExtraArgsScript, 'arg'],
    spawnOptions,
  )
  stderr = result.stderr.toString()
  arrayEq JSON.parse(result.stdout), [
    'coffee',
    initialSpaceExtraArgsScript,
    'arg',
  ]
