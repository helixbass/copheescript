...
test 'Verify OUTDENT tokens are located at the end of the previous token', ->
  ...
  '''
  tokens = CoffeeScript.tokens source
  [..., number, curly, outdent1, outdent2, outdent3, bracket, terminator] =
    tokens
  eq number[0], 'NUMBER'
...
test 'Values with properties end up with a location that includes the properties', ->
  ...
  [singleProperty, twoProperties, indexed, complexIndex] = block.expressions
...
test 'StringWithInterpolations::fromStringLiteral() assigns correct location to tagged template literal', ->
  checkLocationData = (
    source,
    {stringWithInterpolationsLocationData, stringLocationData},
  ) ->
    block = CoffeeScript.nodes source
    ...
    for field in [
      ...
      eq(
        stringWithInterpolations.locationData[field],
        stringWithInterpolationsLocationData[field],
      )
